...
function R_min = HE_RT(dag, f, m)
    ...
    desc_path_lens = sort(dag.lengths, 'descend');

    % skip_longest: avoid removing the length of the longest path twice (already apart of Lfmax)
    skip_longest = 1; 

    for j = 0:k
        path_len_sum = sumPathLengths(dag, index, desc_path_lens, skip_longest, j);
...
function path_len_sum = sumPathLengths(dag, index, desc_path_lens, skip_longest, j)
    ...
    for p = 1:j
        ...
        if skip_longest == 1
            if dag.lengths(index) == desc_path_lens(p)
                skip_longest == 0;
                continue;
            end
        end 
        path_len_sum = path_len_sum + desc_path_lens(p);
    end
end
