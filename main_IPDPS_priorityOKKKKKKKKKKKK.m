...
ACC=1; LS=0; WS=1;US=1;

if(ACC==1)
    s=sprintf('m%d.fig',m);
else
s=sprintf('m%dPriority6n100B0.1.fig',m);
end
...
vectorU_RTAFP_WS = zeros(1, (Umax - Umin) / stepU);
vectorU_RTAFP_HLPLUS_WS = zeros(1, (Umax - Umin) / stepU);

vectorU_RTAFPLS_LS = zeros(1, (Umax - Umin) / stepU);
vectorU_RTAFP_HLPLUS_LS = zeros(1, (Umax - Umin) / stepU);
...
for x = 1 :  nTasksets
 ...
 disp(tasksetsPerU);
    ...
    task = task(ind);
       ...
       for i = 1 : nTasks
        [~,NN]=New_MakeSpan_2(task(i).v, task(i).D, 0);
         task(i).mksp=NN; 
     end;
    for i = 1 : nTasks
    ...
    CK=[];
if(ACC==0)   
    for (ii=1: length(task))
        if(LS==1)
            CK(ii)=task(ii).D-task(ii).len;
        end
        if(WS==1)
            if(US==0)
                CK(ii)=task(ii).D-task(ii).wcw/m;
            else
                CK(ii)=task(ii).wcw/task(ii).T;
            end
        end
        if(LS==0 && WS==0)
        if(task(ii).len<task(ii).wcw/m)
           CK(ii)=task(ii).D-task(ii).wcw/m;
        end
        if(task(ii).len>=task(ii).wcw/m)
           CK(ii)=task(ii).D-task(ii).len;
    ...
    task = task(ind);
     ...
     for i = 1 : nTasks
        [task(i).R, schedRTAFP_WS] = runRTA_FP_2(i, task(i).mksp);
        if schedRTAFP_WS == 0
            break;
        end
    end
        ...
        for i = 1 : nTasks
        [task(i).R, schedRTAFP_HLPLUS_WS] = runRTA_FP_2_HLPLUS(i,task(i).mksp);
        if schedRTAFP_HLPLUS_WS == 0
            break;
        end
        end
    ...
    if schedRTAFP_WS == 1
        vectorU_RTAFP_WS(1, indexU) = vectorU_RTAFP_WS(1, indexU) + 1;
    end

   
    if schedRTAFP_HLPLUS_WS == 1
        vectorU_RTAFP_HLPLUS_WS(1, indexU) = vectorU_RTAFP_HLPLUS_WS(1, indexU) + 1;
    end
    ...
    %if rem(x, save_rate) == 0
        %save(workspace_name, 'x', 'vectorU_RTAFP', 'vectorU_RTAFP_HLPLUS','vectorU_RTAFP_WS', 'vectorU_RTAFP_HLPLUS_WS');
    %end
    if(x==nTasksets)
       break; 
    end
...
y3=vectorU_RTAFP_WS/tasksetsPerU;
y4=vectorU_RTAFP_HLPLUS_WS/tasksetsPerU;
...
xlabel('Total Utilization (U)');
ylabel('Acceptance Ratio');
legend('Our-HUF', 'Our-DM','MBBMB-HUF', 'MBBMB-DM','Location', 'southwest');
