...
function [R, sched] = runRTA_FP_2_HLPLUS_Limcarry(ind, makespan)
% response-time analysis in the case of FP (with the improved upper-bound on the inter-task interference)
    ...
    while R ~= Rold && R <= task(ind).D
        ...
        if ind > 1
            ...
            diff=[];
            for i = 1 : ind - 1
                NC=get_NC_Interf_2_HLPLUS(i, Rold);
                
                CI=getInterf_2_HLPLUS(i, Rold,makespan);
                ...
                if(CI>NC)
                diff=[diff, CI-NC];
                %R = R + (1 / m) * getInterf_2_HLPLUS(i, Rold);
                end
            end
